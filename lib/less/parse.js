...
function checkBlockdecl1(i) {
  ...
  if (l = checkCondition(i)) kind = 1;else if (l = checkExtend(i)) kind = 6;else if (l = checkRuleset(i)) kind = 2;else if (l = checkDeclaration(i)) kind = 3;else if (l = checkAtrule(i)) kind = 4;else if (l = checkMixin(i)) kind = 7;else if (l = checkInclude(i)) kind = 5;else return 0;
  ...
  if (i < tokensLength && (l = checkDeclDelim(i))) i += l;else if (kind !== 7) return 0;

  if (l = checkSC(i)) i += l;else if (kind !== 7) return 0;
...
function checkCombinator2(i) {
  ...
  if (type === TokenType.PlusSign || type === TokenType.GreaterThanSign || type === TokenType.Tilde || type === TokenType.Ampersand) return 1;else return 0;
}
...
function checkIdent(i) {
  ...
  if (tokens[i].type === TokenType.HyphenMinus || tokens[i].type === TokenType.Identifier || tokens[i].type === TokenType.DollarSign || tokens[i].type === TokenType.Asterisk) i++;else if (tokens[i].type !== TokenType.CommercialAt) return 0;
  ...
  if (!wasIdent && tokens[start].type !== TokenType.Asterisk && tokens[start].type !== TokenType.CommercialAt) return 0;
...
function checkTset(i) {
  ...
  if (l = checkVhash(i)) tokens[i].tset_child = 1;else if (l = checkAny(i)) tokens[i].tset_child = 2;else if (l = checkSC(i)) tokens[i].tset_child = 3;else if (l = checkOperator(i)) tokens[i].tset_child = 4;else if (l = checkEscapedString(i)) tokens[i].tset_child = 5;
...
function getTset() {
  ...
  if (childType === 1) return getVhash();else if (childType === 2) return getAny();else if (childType === 3) return getSC();else if (childType === 4) return getOperator();else if (childType === 5) return getEscapedString();
}
...
function getVariablesList() {
  var startPos = pos;
  var token = tokens[startPos];
  var x = token.type === TokenType.FullStop ? [] : [getVariable()];
  var line = token.ln;
  var column = token.col;
...
function checkCompoundSelector1(i) {
  ...
  var l = undefined;
  if (l = checkPercentage(i) || checkTypeSelector(i) || checkParentSelectorWithExtension(i) || checkCombinator(i)) i += l;else return 0;

  while (i < tokensLength) {
    var _l2 = checkShash(i) || checkClass(i) || checkAttributeSelector(i) || checkPseudo(i) || checkIdent(i);
...
function getCompoundSelector1() {
  ...
  if (checkPercentage(pos)) sequence.push(getPercentage());else if (checkTypeSelector(pos)) sequence.push(getTypeSelector());else if (checkParentSelectorWithExtension(pos)) sequence = sequence.concat(getParentSelectorWithExtension());else if (checkCombinator(pos)) sequence.push(getCombinator());

  while (pos < compoundSelectorEnd) {
    if (checkShash(pos)) sequence.push(getShash());else if (checkClass(pos)) sequence.push(getClass());else if (checkAttributeSelector(pos)) sequence.push(getAttributeSelector());else if (checkPseudo(pos)) sequence.push(getPseudo());else if (checkIdent(pos)) sequence.push(getIdent());
  }
